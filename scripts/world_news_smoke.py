...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run an explicit world-news smoke check.")
    parser.add_argument("--source", help="Registered source ID from data/world/sources.json.")
    parser.add_argument("--topic", default="", help="Optional RSS filter; required by GDELT.")
...
async def run(args: argparse.Namespace) -> int:
    ...
    if not args.source:
        ...
        return 0
    try:
        result = await runtime.news(args.source, topic=args.topic, limit=args.limit)
