...
3. **运行时**：`WorldRuntime` 注册 3 类适配器（`AmapWorldSourceAdapter`：IP 定位/实时天气/预报/行政区划/地点/驾车/公交/步行路线；`GdeltNewsAdapter`/`OfficialRssNewsAdapter`/`GovCnPolicyAdapter`：`app/world/adapters/news.py`；`QweatherWeatherAdapter`：`app/world/adapters/qweather.py`）。每个来源必须同时满足：全局开关、来源 `enabled`、`legal_approved`（清单 `data/world/sources.json` + `WORLD_SOURCE_LEGAL_APPROVED_IDS`）与 `WorldAcquisitionService` 门控。
