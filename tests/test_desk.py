...
def test_desk_static_routes_publish_the_sandbox() -> None:
    ...
    assert 'new URLSearchParams(location.search).get("prompt")' in script.text
    assert 'location.assign("/me")' in script.text
    ...
    assert "min-height: 75vh" in style.text
    assert "height: 64px" in style.text
    ...
    assert "desk-shell-v14" in worker.text
...
def test_auth_entry_is_modal_compatible_and_keeps_real_forms() -> None:
    ...
    assert 'class="testimonial-carousel"' in page.text
    ...
    assert "function showTestimonial" in script.text
